...
api.interceptors.response.use(
  (response) => {
    console.log(`📥 ได้รับการตอบกลับ: ${response.status} ${response.statusText}`);
    return response;
  },
  (error) => {
    console.error(`❌ ข้อผิดพลาด API: ${error.response?.status} ${error.response?.statusText}`);
...
export const adminAPI = {
  ...
  login: async (credentials) => {
    try {
      ...
      console.error('❌ เข้าสู่ระบบผู้ดูแลล้มเหลว:', error.response?.data?.message || error.message);
      throw error;
    }
  },
...
export const customerAPI = {
  ...
  login: async (credentials) => {
    try {
      ...
      console.error('❌ เข้าสู่ระบบลูกค้าล้มเหลว:', error.response?.data?.message || error.message);
      throw error;
    }
  },
  ...
  getCategories: async () => {
    try {
      ...
      if (error.response?.status === 500) {
        console.warn('⚠️ Server Error 500 - ใช้ข้อมูลหมวดหมู่เริ่มต้น');
        
        // ส่งคืนข้อมูลหมวดหมู่เริ่มต้น
        return {
          data: {
            success: true,
            data: [
              ...
              },
              {
                id: 2,
                name: 'ของใช้',
                description: 'ของใช้ในบ้านและของใช้ส่วนตัว',
                product_count: 5,
                created_at: new Date().toISOString(),
                updated_at: new Date().toISOString()
              },
              ...
              },
              {
                id: 4,
                name: 'อิเล็กทรอนิกส์',
                description: 'อุปกรณ์อิเล็กทรอนิกส์และแกดเจต',
                product_count: 4,
                created_at: new Date().toISOString(),
                updated_at: new Date().toISOString()
              },
            ...
            message: 'ใช้ข้อมูลหมวดหมู่เริ่มต้น (Database ไม่พร้อม)'
          },
          status: 200,
...
export const orderAPI = {
  createOrder: (orderData) => {
    console.log('📝 สร้างคำสั่งซื้อใหม่:', orderData);
    return api.post('/orders', orderData);
  },
  
  getOrder: (orderNumber) => {
    console.log('🔍 ดึงข้อมูลคำสั่งซื้อ:', orderNumber);
...
export const testDatabaseConnection = async () => {
  try {
    ...
    console.log('✅ ทดสอบฐานข้อมูลเสร็จสิ้น:', { successCount, totalTests, tests });
